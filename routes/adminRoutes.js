...
const overrideAdmin = process.env.OVERRIDE_ADMIN;

console.log("adminRoutes: overrideAdmin", overrideAdmin);

router.use(checkIfAuthenticated, function (req, res, next) {
  if (req.user.role === "admin" || overrideAdmin === "true") {
    console.log("adminRoutes: checkIfAuthenticated passed", overrideAdmin);
    return next();
  }
